...
<head>
  <meta charset="UTF-8" />
  <title>Mis Finanzas 2.0 · App</title>
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <!--
    Este archivo expone window.initUserApp(root, user)
    y contiene toda la lógica de la app (storage por usuario, eventos, cálculos, etc.).
    El HTML/CSS principal ya está en tu index.html.
  -->
  <style>
    ...
    #mf-signout-fixed:hover { filter: brightness(0.95); }
  </style>
</head>
<body>
  <script>
  ...
  // -------------------------------------------------------------
  // NOTA:
  // - Este script asume que el HTML con todos los IDs ya está en index.html
  //   (header, tablas, inputs, etc.) y que "root" es #app-root.
  // - Usa storage por usuario: mf2.${user.uid}.*
  // - Agrega botón de "Cerrar sesión" fijo arriba a la derecha.
  // -------------------------------------------------------------
  window.initUserApp = function(root, user){
    // -----------------------------------------
    // Botón fijo "Cerrar sesión" (si no existe)
    // -----------------------------------------
        ...
        b.addEventListener('click', () => {
          try {
            if (window.firebase && firebase.auth) {
              ...
              // fallback por si no está firebase en este contexto
    ...
    // -----------------------------------------
    // Tu APP: lógica completa (idéntica a la que pegaste en index),
    // solo que vive aquí para mantener el index más limpio.
    // Usa los mismos IDs y elementos ya presentes en el DOM.
    // -----------------------------------------
    ...
    // ====== Elementos (tomados del DOM de index) ======
    // Header / filtros / config
    const selMes=$("#selMes");
    ...
    btnGuardarConfig.onclick=()=>{
      ...
      if(old!==data.config.dolar){
        data.usdHist.push({ts:Date.now(),valor:data.config.dolar});
      }
    ...
    fileImport.onchange=e=>{
      ...
      r.onload=ev=>{
        try{
          ...
          data=obj;guardarData();initAfterDataChange();alert("Importado ✅");
    ...
    function renderPresupuestoPorSubcat(){
      ...
      Object.keys(data.categorias).forEach(cat=>{
        ...
        if(subs.length===0){
          ...
          subs.forEach(sub=> makeRow(`${cat}::${sub}`, sub));
        }
      });
    }
    ...
    function renderInflacion(){
      ...
      const top=25,bottom=h-30,ch=bottom-top;
